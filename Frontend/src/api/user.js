...
export const userPOST = async(firstName,lastName,email,password,roleId  ) =>{
...
export const updateUser = async(username,Columns,Values)=>{
  ...
  try {
    ...
    const response = await fetch(config.url + 'adduser?' + `username=${username}` + '&&' + concatenatedColumns + '&&' + concatenatedValues,{
        method: "PATCH"
      });
        data = await response.json();
        console.log(data)
