...
def insert_database(
    cursor, connection, sql_values, month, year, current_datetime, user
):
    try:
        print("sql_values")
        if sql_values:
...
def create_dataframe(parserChoice):
    ...
    if parserChoice == "0":
        df = df.rename(
            columns={
                "WABA Name / ID / PO": "Name",
                "Destination Country/Region & Product": "Product",
            }
        )
    else:
...
def computing_totals(data_table, pdf_file, sql_values, parserChoice, username):
    try:
        ...
        for i in range(len(data_table)):
                ...
                if i != 0:
                    sql_values.append(
                        (
                            ...
                            org_name,
                            waba_id,
                            int(user_initiated_count),
                            round(user_initiated_total, 2),
                            int(business_initiated_count),
                            round(business_initiated_total, 2),
                            int(authentication_count),
                            round(authentication_total, 2),
                            int(service_count),
                            round(service_total, 2),
                            int(marketing_count),
                            round(marketing_total, 2),
                            int(utility_count),
                            round(utility_total, 2),
                            current_datetime,
                            username,
                            current_datetime,
                            username,
            ...
            product = (
                data_table["Product"].iloc[i].split("-")
            )  # separating region from product
...
def run(sql_values, parserChoice, user):
    try:
        ...
        sql_values, month, year, current_datetime = computing_totals(
            data_table, pdf_file, sql_values, parserChoice, user
        )
        cursor, connection = establish_connection()
        response = insert_database(
            cursor, connection, sql_values, month, year, current_datetime, user
        )
