...
from Models.getOneUser import oneUser
    ...
    try:
        ...
        if request.method == "POST":
            ...
            if checkBillingLogs(parserChoice) == True:
                ...
                if parserChoice == "0":
                    # old pdf reader
                    invoice_number, invoice_month, invoice_year = oldGetVariables(
                        fileName
                    )
                ...
                return (
                    jsonify({"message": "File Uploaded Successfully", "status": 200}),
                    200,
                )
            else:
                # updateProgress(socketio,"ascsac",90)
...
def login():
    try:
        if request.method == "POST":
            ...
            if token != 0:
                return (
                    jsonify({"token": token, "roleName": roleName, "user": user}),
                    200,
                )
            else:
                return (
                    jsonify({"message": "Incorrect Credentials or Account not Active"}),
                    401,
                )
    ...
    try:
        ...
        if param1 and param2:
            ...
            return (
                jsonify(
                    {
                        "message": "Please provide both param1 and param2 as query parameters."
                    }
                ),
                400,
            )
        ...
        if param1 and param2:
            ...
            return (
                send_file(
                    ...
                    download_name=f"{param1+param2}.xlsx",
                ),
                200,
            )
        else:
            raise Exception(
                "Please provide both param1 and param2 as query parameters."
            )
    except Exception as e:
        ...
        if param1 and param2:
            ...
            return (
                send_file(
                    ...
                    download_name=f"{param1 + param2}.pdf",
                ),
                200,
            )
        else:
...
def forgetPassword():
    try:
        ...
        if token != 0:
            msg = Message(
                "Password Reset", sender="haziq.ahmed@eocean.com.pk", recipients=email
            )
    ...
    try:
        ...
        if request.method == "POST":
            ...
            if len(columns) != len(values):
                return (
                    jsonify({"error": "Number of columns and values must match"}),
                    400,
                )
    ...
    try:
        ...
        if param1 and param2:
            ...
            if os.path.exists(file_path):
                ...
                return (
                    jsonify({"message": "File deleted successfully", "status": 200}),
                    200,
                )
            else:
...
def DeleteMau(user, permissions, role):
    try:
        ...
        if param1 and param2:
            ...
            if os.path.exists(file_path):
                # deleting from Database
                response = DeleteBillingMAU(param1, param2)
...
@token_required
def getOneUser(user, permissions, role, user_email):
    try:
        print(user_email)
        response = oneUser([user_email])
