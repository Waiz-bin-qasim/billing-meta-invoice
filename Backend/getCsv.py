...
def run(param1 , param2):
	...
	for record in result_list:
		estimatedUSD += float(record[18])
		if(record[11]=='USD'):
			estimatedUSD += float(record[12])
		else:
			estimatedPKR += float(record[12])
		if(record[19] == 'USD'):
			estimatedUSD += float(record[20])
		else:
			estimatedPKR += float(record[20])
		if(record[16]>0):
			estimatedUSD += (float(record[15])*float(record[16]))
